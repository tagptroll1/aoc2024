...
fn lines_to_matrix(lines: &Vec<String>) -> Vec<Vec<char>> {
     lines.iter().map(|x| x.chars().collect::<Vec<char>>()).collect::<Vec<Vec<char>>>()
}
...
fn search(input: &Vec<Vec<char>>, word: &Vec<char>, x: i16, y: i16, dir: &(i16,i16)) -> i64 {
    ...
    // Validate bounds
    if (input.len() as i16) <= y_bound || y_bound < 0{
        return 0;
    }

    if y < 0 || (input[y as usize].len() as i16) <= x_bound || x_bound < 0 {
        return 0;
    }
...
mod tests {
    ...
    fn test_part_1() {
        let expected = 18;
        assert_eq!(expected, part_one(&TEST_INPUT.map(|s| s.to_string()).to_vec()));
    }

    #[test]
    fn test_part_2() {
        let expected = 9;
        assert_eq!(expected, part_two(&TEST_INPUT.map(|s| s.to_string()).to_vec()));
    }
}